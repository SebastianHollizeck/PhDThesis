...
All samples apart from sample 2 showed whole genome duplication and high polyclonality, which suggests that in addition to the heterogeneity observed through short and structural variants there was an additional level of heterogeneity of copy number alterations. \add{However with the extremely low purity of sample 2, which directly conflicted with the H\&E staining result (}\autoref{tab:ca80wgsSamples}\add{), our standard copynumber workflow did not produce satisfying results. As manual inspection of the likelihood contours showed no alternative solutions and the depth depth ratios were irregular but acceptable(}\Autoref{fig:ca80.2input,fig:ca80.2contour}\add{, we considered sample 2 as low tumour purity and considered downstream analysis results as low confidence.} The lower purity of sample 2 might have been a confounding factor, however both samples 28 and 32 showed lower purities but a much higher polyclonality and genome duplication.  \note{info about the smoothing attempts}As both these samples had a very fragmented copy number profile, we tweaked the smoothing parameters of the PURPLE pipeline, by setting the \lq\textit{pcf\_gamma}\rq\ parameter of COBALT to three times the default (300) and allowed purple to smooth over three times more multiple copy number segments with the \lq\textit{min\_diploid\_tumor\_ratio\_count}\rq. This change reduced the spikiness of the copy number segments slightly. Nevertheless, the local copy number calls should be considered low confidence.
