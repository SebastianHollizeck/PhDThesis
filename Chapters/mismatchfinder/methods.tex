...
To enable a z-score like estimation, we calculate the CDF-value (\autoref{eq:betaCDF}) for each patient samples $p$ signature weight $w$ of signature $s$ with healthy sample fitted shape parameters $\alpha_s$ and $\beta_s$ per signature. This allows us to estimate how many healthy samples would have a signature weight less than the patient sample for the respective signature $s$ (\autoref{eq:sigCDFScore}). While we did not see significant down regulation of signatures from the background, the method supports both over- and under-re\-pre\-sentation analysis. Ten percent of the tail of each signature were removed before fitting the shape parameters to minimise the impact of outliers.
...
\begin{equation}
\forall s \in Signatures,\ \forall p \in Patients:\ \text{CDF-score}(s,p) := F(w_s(p); \alpha_s ,\beta_s)
\label{eq:sigCDFScore}
\end{equation}
\myequation[\ref{eq:sigZScore}]{MisMatchFinder: CDF-score calculation per signature and patient}

With a CDF-value $\geq 0.95$ we assumed a signature to be active over the normal background.
...
\todo[color=green,inline]{This is basically calculating the quantile and using that as a cutoff, do i rewrite that}

\subsection{Tumour detection}
\label{mmf-sec:tumourdetection}

\todo[inline]{write about the glmnet and compare to ichorCNA}

ichorCNA tumour purity > 3\% vs GLMnet